...
import { FormWarehouseModal } from '@/components/FormWarehouseModal'
...
function WarehouseCardSkeleton() {
  return (
    <Card className="overflow-hidden rounded-xl border-none bg-white p-0 shadow-sm dark:border dark:border-white/10 dark:bg-[#15211a]">
      ...
      <CardContent className="space-y-3 p-4">
        ...
        <div className="h-2 w-full animate-pulse rounded-full bg-gray-200 dark:bg-white/10" />
...
function RouteComponent() {
  ...
  // Ambil daftar gudang milik user (owner-scoped di backend).
  const fetchData = async () => {
    setLoading(true)
    try {
      const res = await api.get<{ data: Array<IWarehouseResponse> }>(
        '/warehouses',
      )
      setWarehouses(res.data.data.map(mapWarehouse))
    } catch (error) {
  ...
  const stats = useMemo(() => {
    // Total Barang = Σ stok (unit) seluruh gudang; kapasitas dihitung dari stok.
    const totalStock = warehouses.reduce((sum, w) => sum + w.stockTotal, 0)
    const totalCapacity = warehouses.reduce((sum, w) => sum + w.capacity, 0)
    const nearlyFull = warehouses.filter(
      (w) => usagePercent(w.stockTotal, w.capacity) >= 80,
    ).length
    const usage = totalCapacity
      ? Math.round((totalStock / totalCapacity) * 100)
      : 0
    return { totalStock, nearlyFull, usage }
  }, [warehouses])
    ...
    <div className="min-h-screen w-full bg-[#e4f0e4] dark:bg-[#0c1410]">
      ...
      <div className="relative z-10 mx-auto -mt-14 max-w-6xl space-y-6 px-4 pb-16 sm:px-6 lg:px-10">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {!loading &&
            pageItems.map((w) => {
              const usage = usagePercent(w.stockTotal, w.capacity)
                  ...
                  <CardContent className="space-y-3 p-4">
                    <div className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          {w.items} jenis · {w.stockTotal} unit
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
        ...
        {!loading && filtered.length > 0 && (
          <Pagination
            ...
            summary={`Menampilkan ${from}–${to} dari ${total} gudang`}
          />
        )}
